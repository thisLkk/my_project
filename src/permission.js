    ...
    if (to.path === '/login') {
      ...
      if (store.getters.roles.length === 0) { // 判断当前用户是否已拉取完user_info信息
        ...
        store.dispatch('GetUserInfo').then(() => { // 拉取user_info
          //拉取字典
          return store.dispatch('itemList')
        }).then(()=>{
          //加载业务日期
          return store.dispatch('loadBizDate')
        }).then(() => {
          //从store里面取menu信息
          return store.dispatch('GetUserMenus')
        }).then(res => {
          var navmenus = res.results
          return store.dispatch('GenerateRoutes', { navmenus })
          ...
          store.dispatch('FedLogOut').then(() => {
            if (err && err.length > 0) {
              Message.error('登录认证错误：' + err)
            } else {
              Message.error('登录认证错误，请重新登录')
            }
            next({ path: '/login' })
          })
        })
