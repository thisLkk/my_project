<template>
  <!-- 协同管理 _ 我的协同商机 _ 提交 -->
  <div class="yh_BusinessSubmitForm">
...
export default {
  ...
  data() {
    return {
      ...
      businessName: "", //--------------------------协同机构传值给组件
      placeholder: "正式与注册客户选其一",
      public_info: {
        lianxiren_name: "",
        faqiren_name: "",
        lianxiren_tel: "",
      ...
      ResponseModeList: dictItems(DICTYPE.OSPCD0231), //-----------协同商机模式
      ServiceCateList: dictItems(DICTYPE.OSPCD0227), //------------服务类别
      BusinessCateList: dictItems(DICTYPE.OSPCD0226), //-----------业务类别
      ...
      EchoMyCollaboration(query)
        .then(response => {
          if (response.Meta.Code == "0000") {
            ...
            var date = new Date(response.Data.InitiateDate); //--------------------------------发起人时间
            var nowDate =
              date.getFullYear() +
              "-" +
              (date.getMonth() + 1) +
              "-" +
              date.getDate();
            var date = formatDateTime(response.Data.InitiateDate, "yyyy-MM-dd");
