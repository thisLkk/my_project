<template>
  <!-- 协同管理 _ 我的协同商机 _ 审批 -->
  <div class="yh_ExaminationAndApproval">
    ...
    <div class="btn">
      ...
      <el-button @click="newForm_close()" size="mini">
        <i class="fa fa-close diologBtn"></i> 拒绝申请</el-button>
    </div>
...
export default {
  ...
  data() {
    return {
      ...
      CoordinationList: dictItems(DICTYPE.OSPCD0231),
  ...
  created() {
    ...
    this.bussines_ending.time = nowDate;
  ...
  methods: {
        ...
        .then(response => {
          if (response.Meta.Code == "0000") {
            this.public_info = {
              ...
              faqiren_jigou: response.Data.OrgFullName, //此处需要拿到码值进行转换
              formal: {
                custName: "",
                custNo: "",
                custType: ""
            ...
            this.bussines_info.expectedData_ = new Date(
    ...
    //清空
    resetForm() {
      this.bussines_info.splice(1); //-------------------------------发送完接口在清空  暂时不清
