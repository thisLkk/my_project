<template>
  ...
  <div class="yh_BusinessTable">
    <div class="BusinessTable_title clearfix">
      ...
      <router-link class="fr" to='/ResourceManage/KnowledgeBaseManagement/KnowledgeBaseQuery'>
        去知识库查看更多业务资料...
      </router-link>
      </div>
    <div class="main_table">
      <el-table :data="list" stripe size="mini" max-height=800 style="width: 100%">
        ...
        <!-- <el-table-column label="业务类别" width="" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              ...
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">
                  <a href="javascript:void(0)" @click="business_(scope.row)">{{bizCateFormatter(scope.row.BizCate)}}</a>
                </el-tag>
              </div>
            </el-popover>
          </template>
        ...
        <el-table-column label="商机类别" width="" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              ...
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">{{ buopKnlgCateFormatter(scope.row.BuopKnlgCate) }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="服务类别" width="" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p>{{ srvCateFormatter(scope.row.SrvCate) }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">{{ srvCateFormatter(scope.row.SrvCate) }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        ...
        <el-table-column label="对接人" width="" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              ...
              <div slot="reference" class="name-wrapper">
                <a href="javascript:void(0)" @click="business_(scope.row)">{{scope.row.AlignPer}}</a>
        ...
        <el-table-column label="职务" width="" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p>{{ scope.row.AlignPerDuty }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">{{ scope.row.AlignPerDuty }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        ...
        <el-table-column label="手机号" width="100" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p>{{ scope.row.MobNo }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">{{ scope.row.MobNo }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
        <el-table-column label="联系邮箱" width="120" align="center">
          <template slot-scope="scope">
            <el-popover trigger="hover" placement="top">
              <p>{{ scope.row.ContactEmail }}</p>
              <div slot="reference" class="name-wrapper">
                <el-tag size="mini">{{ scope.row.ContactEmail }}</el-tag>
              </div>
            </el-popover>
          </template>
        </el-table-column>
    ...
    <!---------------------------------- 业务通讯录新建 ---------------------------------------->
    <div class="dialog_">
      <el-dialog @close="doCancle" title="业务通讯录新增" id="dialog_add" :visible.sync="addShow" v-if="addShow" width="800px">
        <div class="activity_dialog clearfix yh-jbxx-content ">
          <el-form :rules="addRules" :model="form_data_add.param" ref="form_data_add" label-width="100px" size=" mini">
              ...
              <el-col :span="12">
                <el-form-item label="商机类别:" prop="buopKnlgCate">
                    <el-select @change="buopKnlgCateChange()" v-model="form_data_add.param.buopKnlgCate" placeholder="请选择" class="yh-dialogselect" size="mini">
                      <el-option v-for="item in items_sjlb" :key="item.value" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                </el-form-item>
              </el-col>
            ...
            <el-row>
              ...
              <el-col :span="12">
                <div class="yh-timeinput">
                  <el-form-item label="服务类别:" prop="SrvCate">
                    <el-select v-model="form_data_add.param.SrvCate" placeholder="请选择" class="yh-dialogselect" size="mini">
    ...
    <!------------------------------------ 业务通讯录详情 ---------------------------------->
    <div class="dialog_">
      <el-dialog title="业务通讯录详情" :visible.sync="viewShow" width="800px">
        <div class="activity_dialog clearfix yh-jbxx-content ">
          <el-form v-if="form_data_view!=null" :model="form_data_view" ref="form_data_view" label-width="100px" size=" mini">
            <el-row>
              <el-col :span="12">
                <el-form-item label="业务类别:" prop="BizCate">
            ...
            <el-row>
              ...
              <el-col :span="12">
                <div class="yh-timeinput">
                  <el-form-item label="服务类别:" prop="SrvCate">
                    {{srvCateFormatter(form_data_view.SrvCate)}}
                  </el-form-item>
                </div>
              </el-col>
            </el-row>
...
import { babManageEdit,babManageDel,babManageAdd, 
...
import {JSONValue2JSON,JSONValue2Null} from "@/utils/datahandle.js";
...
*/
      ...
      items_sjlb:dictItems(DICTYPE.OSPCD0224), //商机类别,
  ...
  methods: {
    ...
    srvCateFormatter(code){
      return dictFilter(DICTYPE.OSPCD0227,code);
    },
    //商机改变
    buopKnlgCateChange(){
      //控制联动    公司部门跟随商机类别
    ...
    handleDelete(index, rowId) {
      ...
      // //console.log(index, row);
        ...
        this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
          ...
          babManageDel(this.form_data_del)
          .then(response => {
            this.$message({
              type: 'success',
              message: '删除成功!'
    ...
    new_dialog() {
      ...
      this.addShow = true;
      //控制dialog弹窗title
      $("#dialog_add span.el-dialog__title").html("业务通讯录新增");
    ...
    edit_dialog(index,row) {
      ...
      $("#dialog_add span.el-dialog__title").html("业务通讯录编辑");
    },
    //每页数量改变
    ...
    doSave(){
        ...
        if (valid) {
          if(this.addFlag){//新增
               babManageAdd(this.form_data_add)
              .then(response => {
                this.$message({
                  type: 'success',
                  message: '新增成功!'
              ...
              babManageEdit(this.form_data_add)
              .then(response => {
                this.$message({
                  type: 'success',
                  message: '修改成功!'
    ...
    doCancle(){
      ...
      this.form_data_add.param.fileList = [];
    },
  },
...
/*ouxinsu补充按钮样式*/
.el_btn .el-button--mini.is-round,.main_table .el-button--mini{
  padding:2px 9px;
  margin-left:10px;
}
