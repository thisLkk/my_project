    ...
    <div class="calendar">
        <!-- 弹出框，显示当天活动内容 -->
         <div class="SmallFloat">
            <div class="ActiveHeaderOne">
                <a href="javascript:;" class="ActiveHeaderOneXin">新建</a>
                <a href="javascript:;" class="ActiveHeaderOneCha">查看更多>></a>
            </div>
            <div class="ActiveHeaderTwo">
                <p></p>
                <a href="javascript:;" class="ActiveHeaderTwoYu">业务推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoZhong">重点进行产品推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoBei">北京***企业有限公司</a>
            </div>
            <div class="ActiveHeaderThree">
                <p></p>
                <a href="javascript:;" class="ActiveHeaderTwoYu">业务推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoZhong">重点进行产品推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoBei">北京***企业有限公司</a>
            </div>
            <div class="ActiveHeaderFour">
                <p></p>
                <a href="javascript:;" class="ActiveHeaderTwoYu">业务推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoZhong">重点进行产品推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoBei">北京***企业有限公司</a>
            </div>
            <div class="ActiveHeaderFive">
                <p></p>
                <a href="javascript:;" class="ActiveHeaderTwoYu">业务推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoZhong">重点进行产品推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoBei">北京***企业有限公司</a>
            </div>
            <div class="ActiveHeaderSixe">
                <p></p>
                <a href="javascript:;" class="ActiveHeaderTwoYu">业务推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoZhong">重点进行产品推荐</a>
                <a href="javascript:;" class="ActiveHeaderTwoBei">北京***企业有限公司</a>
            ...
            <div class="calendar-info" @click.stop="changeYear">
                ...
                <div class="month">
                    <div class="month-inner" :style="{'top':-(this.month*20)+'px'}">
                        <span :key="index" v-for="(m,index) in months">{{m}}</span>
                    </div>
                </div>
        ...
        <thead>
            <tr>
                <td :key="index"  v-for="(week,index) in weeks" class="week">{{week}}</td>
            </tr>
        </thead>
        <tbody>
        <tr :key="k1" v-for="(day,k1) in days" style="{'animation-delay',(k1*30)+'ms'}">
            <td :key="k2" v-for="(child,k2) in day" :class="{'selected':child.selected,'disabled':child.disabled}" @click="select(k1,k2,$event)">
                <span class="actives" :class="{'red':k2==0||k2==6||((child.isLunarFestival||child.isGregorianFestival) && lunar)}" >{{child.day}}</span>
        ...
        <div class="calendar-years" :class="{'show':yearsShow}">
            <span :key="index" v-for="(y,index) in years" @click.stop="selectYear(y)" :class="{'active':y==year}">{{y}}</span>
        </div>
...
<script>
import { getactivitycalendarlist } from "@/api/organization/dashboard/index.js";
...
export default {
  ...
  mounted() {
    var _self = this;
    _self.init();
    _self.$on("callbackMSgData", function(data) {
      //接受父类选中有活动的日期 进行渲染
    });
  },
  methods: {
    ...
    inits() {
      ...
      this.formatmyCalentData(nowMonth, nowYear);
    },
    formatmyCalentData: function(mouth, year) {
      var _self = this;
      ...
      //当月第一天转为yyyy-mm-dd形式
      var times = getCurrentMonthFirst();
      var nowtimeStringone =  times.getFullYear() + "-" + (times.getMonth() + 1) + "-" + times.getDate();
        // console.log(nowtimeStrings)
      //获取当前月的最后一天
      ...
      //转为yyyy-mm-dd形式
      var time = getCurrentMonthLast();
      var nowtimeString =  time.getFullYear() + "-" + (time.getMonth() + 1) + "-" + time.getDate();
      var nextday=nowtimeString.split("-");
      var oneday =nowtimeStringone.split("-");
      var  str ="0"
      if((nextday[1]) < 10){
        var   ting = str + nextday[1]
      }
      if((oneday[1]) < 10){
        var   tingone = str + oneday[1]
      }
      if((oneday[2]) < 10){
        var   tingtwo = str + oneday[2]
      }
      //刷新页面后向后端发送的数据
      var oneString =  time.getFullYear() + "-" + tingone + "-" + tingtwo;
      var twoString =  time.getFullYear() + "-" + ting + "-" + time.getDate();
           //console.log( oneString )
          // console.log(twoString)
            this.query.param.PlanBeginTime = oneString ;
            this.query.param.PlanEndTime = twoString;

      // var lastname = [];
      // for(var i=1; i<time.getDate()+1;i++){
      ...
      //进入页面将当月第一天与当月最后一天发送给后端格式(AA-BB-CC)

      getactivitycalendarlist(this.query).then(response =>{
      ...
      //点击next(),将下个月的第一天与最后一天以(AA-BB-CC)形式发送给后端
      //点击prev(),将上个月的第一天与最后一天以(AA-BB-CC)形式发送给后端
      var y = this.year;
      var m = this.month + 1;
      var n = "31";
      if (m == "4") {
      ...
      for (i = 1; i <= lastDateOfMonth; i++) {
        ...
        if (this.range) {
          ...
          let options = Object.assign(
            ...
            console.log(this.year, this.month + 1, i)
          );
    ...
    prev(e) {
      ...
      this.render(this.year, this.month);
      this.$emit("selectMonth", this.month + 1, this.year);
      this.formatmyCalentData(this.month + 1, this.year);
    ...
    next(e) {
      ...
      // var nowDatesx =this.year+ "-" + this.month + "-" + date.getDate();
